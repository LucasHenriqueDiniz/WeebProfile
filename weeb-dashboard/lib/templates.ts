...
import type { SvgStyle, SvgSize, SvgTheme } from "@/types/svg"
...
/**
 * Valores de configuração para SVGs
 */
export const SVG_STYLES: SvgStyle[] = ["default", "terminal"]
export const SVG_SIZES: SvgSize[] = ["half", "full"]
export const SVG_THEMES: SvgTheme[] = ["default", "dark", "purple", "pink", "blue", "green", "dracula"]

/**
 * Helper para verificar se um valor é válido
 */
export function isValidSvgStyle(value: string): value is SvgStyle {
  return SVG_STYLES.includes(value as SvgStyle)
}

export function isValidSvgSize(value: string): value is SvgSize {
  return SVG_SIZES.includes(value as SvgSize)
}

export function isValidSvgTheme(value: string): value is SvgTheme {
  return SVG_THEMES.includes(value as SvgTheme)
}
