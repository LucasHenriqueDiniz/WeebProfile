...
export function selectMissingConfigs(
  ...
  const relevantMissingSecrets = bootstrap.missingSecrets.filter(
    ({ pluginName }) => pluginsToValidate.includes(pluginName)
  )
  ...
  return combined.filter((item, index, self) =>
    index === self.findIndex((t) => t.plugin === item.plugin && t.field === item.field)
  )
}
