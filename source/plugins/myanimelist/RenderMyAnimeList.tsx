...
const sectionRenderers: Record<string, (malData: MalData) => JSX.Element> = {
  ...
  character_simple_favorites: (malData: MalData) => <SimpleFavorites data={malData.favorites.characters} type="characters" />,
...
function RenderMyAnimeList({ plugin, data }: { plugin: MyAnimeListConfig; data: MalData }): JSX.Element {
  ...
  const error = CheckPluginForRequiredValues({ plugin, ENV_VARIABLES: MAL_ENV_VARIABLES, pluginName: "MyAnimeList" });
