...
export async function getImageLastUpdates(data: MalLastUpdatesResponse) {
  ...
  anime = await Promise.all(
    anime.map(async (item) => {
      const imageUrl = item.entry.images.jpg?.image_url || ""
      const base64Image = await imageToBase64(imageUrl)
...
export async function convertFavoritesFullImagesToBase64(
  data: MalFullFavoritesResponse
  ...
  if (manga) {
    manga = await Promise.all(
      manga.map(async (item) => {
        const imageUrl =
          item.images.jpg?.image_url || item.images.jpg?.small_image_url || item.images.jpg?.large_image_url || ""
        const base64Image = await imageToBase64(imageUrl)
...
export async function fetchMalData(plugin: MyAnimeListPlugin, dev = false): Promise<MalData> {
  ...
  const malData: MalData = {
    ...
    last_updated: await getImageLastUpdates({
