...
function RenderFavorites({ data, hideOverlay }: { data: AnyMalFavorite; hideOverlay: boolean | undefined }): JSX.Element {
  return (
...
function SimpleFavorites({ data, type }: DefaultSimpleFavoritesProps): JSX.Element {
  ...
  switch (type) {
    ...
    case "people":
      ...
      break;
    default:
      maxItems = myanimelist.character_favorites_max ?? (MAL_ENV_VARIABLES.character_favorites_max.defaultValue as number);
      hideTitle = myanimelist.character_favorites_hide_title ?? false;
      title = myanimelist.character_favorites_title ?? (MAL_ENV_VARIABLES.character_favorites_title.defaultValue as string);
  }
    ...
    <section id="mal" className="simple-favorites">
      <RenderBasedOnStyle
        ...
        terminalComponent={
          <>
            {!hideTitle && (
              <TerminalCommand command={getPseudoCommands({ plugin: "mal", section: "favorites", username: "username", type, limit: maxItems })} />
            )}
            {data.map((data) => (
              <div className="terminal-grid-2 mb-4" key={data.mal_id}>
                <p className="sm-text text-overflow text-nowrap text-warning">* {"name" in data ? treatJapaneseName(data.name) : data.title}</p>
