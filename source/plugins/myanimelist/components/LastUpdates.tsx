      ...
      <div className="flex-d w100 justify-evenly">
        ...
        <div className="flex justify-between">
          <div className="md-text gap-4 flex items-baseline">
            <span
              className={`default-${
                status === "Reading" ? "watching" : status === "Plan to Read" ? "plan-to-watch" : status.toLowerCase().split(" ").join("-")
              }`}
            >
              {status}
            </span>
  ...
  return (
    <section className="default-last-updates">
      <RenderBasedOnStyle
        ...
        terminalComponent={
          <>
            <TerminalCommand command={getPseudoCommands({ plugin: "mal", section: "last_updates", username: myanimelist.username, limit: maxItems })} />
            <div className="flex-d gap-4">
              {allUpdates.map((update, index) => (
