...
function DefaultCharactersFavorites({ data }: { data: CharacterFavorites[] }): JSX.Element {
  ...
  const maxItems = myanimelist.character_favorites_max ?? (MAL_ENV_VARIABLES.character_favorites_max.defaultValue as number);
  const title = myanimelist.character_favorites_title ?? (MAL_ENV_VARIABLES.character_favorites_title.defaultValue as string);
  ...
  return (
    <section id="mal" className="characters-favorites">
      <RenderBasedOnStyle
        ...
        terminalComponent={
          <>
            <TerminalCommand command={getPseudoCommands({ plugin: "mal", section: "characters_favorites", username: myanimelist.username, limit: maxItems })} />
            <div className="flex-d gap-4">
              {data.map((data, index) => (
