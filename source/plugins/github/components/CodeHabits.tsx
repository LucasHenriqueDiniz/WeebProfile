...
const DefaultCodeHabits = ({ data }: { data: CodeHabitsData }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
          ...
          {Array.from({ length: 24 }).map((_, hour) => {
            ...
            return (
              <div key={hour} className="flex flex-col items-center w-8 h-full">
                <div className="w-2 bg-default-surface rounded-t-md flex items-end h-full justify-end flex-col">
      ...
      <div className="text-xs text-default-muted text-end">Computed from last {data.analyzedCommits || 0} commits</div>
    </div>
  )
}
