...
const sectionRenderers: SectionRenderers = {
  ...
  top_artists_grid: (lastFmData) => <TopArtistsGrid data={lastFmData.topArtists} interval={lastFmData.topArtistsInterval} />,
  top_artists_list: (lastFmData) => <TopArtistsList data={lastFmData.topArtists} interval={lastFmData.topAlbumsInterval} />,
  top_artists_default: (lastFmData) => <TopArtistsDefault data={lastFmData.topArtists} interval={lastFmData.topAlbumsInterval} />,

  top_albums_list: (lastFmData) => <TopAlbumsList data={lastFmData.topAlbums} interval={lastFmData.topAlbumsInterval} />,
  top_albums_grid: (lastFmData) => <TopAlbumsGrid data={lastFmData.topAlbums} interval={lastFmData.topAlbumsInterval} />,
  top_albums_default: (lastFmData) => <TopAlbumsDefault data={lastFmData.topAlbums} interval={lastFmData.topAlbumsInterval} />,

  top_tracks_list: (lastFmData) => <TopTracksList data={lastFmData.topTracks} interval={lastFmData.topTracksInterval} />,
  top_tracks_grid: (lastFmData) => <TopTracksGrid data={lastFmData.topTracks} interval={lastFmData.topTracksInterval} />,
  top_tracks_default: (lastFmData) => <TopTracksDefault data={lastFmData.topTracks} interval={lastFmData.topTracksInterval} />,
};

export default function RenderLastFm({ plugin, data }: Props): JSX.Element {
  // check if has any required value missing
  const error = CheckPluginForRequiredValues({ plugin, ENV_VARIABLES: LASTFM_ENV_VARIABLES, pluginName: "LastFM" });
