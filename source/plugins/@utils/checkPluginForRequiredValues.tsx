...
function CheckPluginForRequiredValues<T, D>({ plugin, ENV_VARIABLES, pluginName }: { plugin: T; ENV_VARIABLES: D; pluginName: string }): JSX.Element | null {
  if (!plugin) {
    return <ErrorMessage message={`${pluginName} plugin is not initialized or found.`} />;
  }

  if (!ENV_VARIABLES) {
    return <ErrorMessage message={`Environment variables (ENV_VARIABLES) for ${pluginName} are missing or not provided.`} />;
  }
