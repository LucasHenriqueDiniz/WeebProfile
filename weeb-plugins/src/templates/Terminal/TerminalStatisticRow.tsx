...
export function TerminalStatisticRow({
  rows,
  className,
}: {
  rows: TerminalStatisticRowProps[]
  className?: string
}): React.ReactElement {
  return (
    <div className={`flex flex-col gap-1 w-full ${className ?? ""}`}>
      {rows.map((row, index) => (
        ...
        >
          {row.icon && <span className="text-terminal-highlight flex-shrink-0">{row.icon}</span>}
          <span className={`text-terminal-warning font-semibold truncate flex-1 ${row.strong ? "font-bold" : ""}`}>
            {row.title}
          </span>
          <span className="text-terminal-muted whitespace-nowrap flex-shrink-0">{row.value}</span>
        </div>
      ))}
    </div>
  )
