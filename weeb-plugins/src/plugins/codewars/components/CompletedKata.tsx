...
export function CompletedKata({
  data,
  config,
  style = "default",
  size = "half",
}: CompletedKataProps): React.ReactElement {
  ...
  const terminalLines: TerminalLineProps[] = limitedData.map((kata) => ({
    right: kata.name,
    left:
      kata.difficulty !== "Unknown"
        ? `${kata.difficulty}${kata.completedAt ? ` (${new Date(kata.completedAt).toLocaleDateString()})` : ""}`
        : kata.completedAt
          ? new Date(kata.completedAt).toLocaleDateString()
          : "",
  }))
