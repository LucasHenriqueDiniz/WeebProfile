...
const DefaultWarning = ({ warnings }: { warnings: NonNullable<GithubData['warnings']> }) => {
  ...
  const priorityOrder: Array<GithubData['warnings'][0]['type']> = ['rate_limit', 'auth_error', 'api_error', 'partial_data']
  ...
  return (
    <div className="flex items-center justify-center py-2 px-3 mt-2">
      <p className="text-xs text-default-muted italic opacity-75">
        {message}
      </p>
    </div>
  )
...
const TerminalWarning = ({ warnings }: { warnings: NonNullable<GithubData['warnings']> }) => {
  ...
  return (
    <div className="py-1 px-2 mt-1">
      <p className="text-xs text-terminal-muted italic opacity-70">
        # {message}
      </p>
    </div>
  )
...
export function GithubWarning({ warnings, style = 'default', size = 'half' }: GithubWarningProps): React.ReactElement | null {
