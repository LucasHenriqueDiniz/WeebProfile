...
import { IMAGE_OPTIMIZATION, urlToBase64 } from "../../../utils/image-to-base64"
...
export async function fetchSpotifyData(
  ...
  console.log(`[Spotify] Fetching data for user: ${essentialConfig?.username || "mock"}`)

  try {
    // Tentar buscar dados reais da API se refresh token disponível
    if (essentialConfig?.refreshToken) {
      ...
      const [recentTracks, topArtists, topTracks, currentlyPlaying, playlists, profile] = await Promise.all([
        hasRecentTracks ? fetchRecentlyPlayed(accessToken, recent_tracks_max) : Promise.resolve([]),
        hasTopArtists ? fetchTopArtists(accessToken, top_artists_period, top_artists_max) : Promise.resolve([]),
        hasTopTracks ? fetchTopTracks(accessToken, top_tracks_period, top_tracks_max) : Promise.resolve([]),
        hasCurrentlyPlaying ? fetchCurrentlyPlaying(accessToken).catch(() => null) : Promise.resolve(null),
        hasPlaylists ? fetchPlaylists(accessToken, playlists_max) : Promise.resolve([]),
        hasProfile ? fetchProfile(accessToken).catch(() => null) : Promise.resolve(null),
      ])
      ...
      console.log(`[Spotify] Successfully fetched data for ${essentialConfig?.username || "user"}`)
      return data
    } else {
      // Sem refresh token, usar dados mock
      console.log(`[Spotify] Using mock data (no refresh token provided)`)
    ...
    console.log(`[Spotify] Using mock data as fallback`)
    const mockData = getMockSpotifyData()
    ...
    for (const [key, value] of Object.entries(data)) {
      if (
        ...
        if (previewMode) {
          ...
          // Converter URL para base64 com otimização (thumbnails)
