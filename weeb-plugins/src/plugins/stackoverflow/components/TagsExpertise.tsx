...
export function TagsExpertise({
  data,
  config,
  style = "default",
  size = "half",
}: TagsExpertiseProps): React.ReactElement {
  ...
  const listItems: ListItemProps[] = limitedData.map((tag) => {
    ...
    return {
      ...
      left: (
        <span className="text-xs font-semibold text-default-highlight">{abbreviateNumber(tag.score)} score</span>
      ) as any,
    }
  })
