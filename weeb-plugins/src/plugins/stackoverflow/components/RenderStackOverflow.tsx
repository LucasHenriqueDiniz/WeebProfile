...
export function RenderStackOverflow({
  ...
  const renderedSections = sections.map((section) => {
    switch (section) {
      ...
      case "badges":
        return <Badges key="badges" badges={data.badges} config={fullConfig} style={style} size={size} />
      ...
      case "tags_expertise":
        return <TagsExpertise key="tags_expertise" data={data.topTags} config={fullConfig} style={style} size={size} />
      default:
        return null
    }
  })

  return <section id="stackoverflow-plugin">{renderedSections.filter(Boolean)}</section>
}
