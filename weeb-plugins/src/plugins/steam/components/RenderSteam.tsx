 ...
 */
export function RenderSteam({ config, data, style = "default", size = "half" }: RenderSteamProps): React.ReactElement {
  ...
  const sectionConfig = {
    ...
    ...Object.keys(config).reduce(
      (acc, key) => {
        ...
        if (key.startsWith("statistics_") || key.startsWith("recent_games_") || key.startsWith("top_games_")) {
  ...
  const renderedSections = sections.map((section) => {
    switch (section) {
      case "statistics":
        return <Statistics key={section} data={data} config={sectionConfig} style={style} size={size} />
      case "recent_games":
        return <RecentGames key={section} data={data} config={sectionConfig} style={style} size={size} />
      case "top_games":
        return <TopGames key={section} data={data} config={sectionConfig} style={style} size={size} />
      default:
        return (
