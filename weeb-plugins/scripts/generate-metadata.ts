        ...
        section.configOptions.forEach((opt, optIndex) => {
          ...
          if (opt.type === "number") {
            ...
            if (opt.min !== undefined && opt.max !== undefined && opt.min > opt.max) {
              errors.push(
                `sections[${index}].configOptions[${optIndex}]: min (${opt.min}) cannot be greater than max (${opt.max})`
              )
            }
          }
        })
...
async function loadPluginMetadata(pluginName: string): Promise<PluginMetadataPartial | null> {
  ...
  try {
    ...
    const exportKey = Object.keys(module).find((key) => key.includes("PluginMetadata") || key.includes("metadata"))
...
function enrichMetadataWithI18n(
  metadata: PluginMetadataPartial,
  pluginId: string,
...
function generateMetadataFile(
  pluginsMetadata: Map<string, PluginMetadataPartial>,
...
`
  ...
  const plugins = Array.from(pluginsMetadata.entries())
    .map(([name, metadata]) => {
      ...
      // Gerar sections
      const sectionsStr = meta.sections
        .map((section, idx) => {
          ...
          if (section.configOptions && section.configOptions.length > 0) {
            const configOptionsStr = section.configOptions
              .map((opt, optIdx) => {
                ...
                if (opt.i18nKey) {
                  ...
                  if (opt.i18nKey.options && Object.keys(opt.i18nKey.options).length > 0) {
                    const optionsI18nStr = Object.entries(opt.i18nKey.options)
                      .map(([value, key]) => `              ${JSON.stringify(value)}: ${JSON.stringify(key)}`)
                      .join(",\n")
                    i18nParts.push(`\n            options: {\n${optionsI18nStr}\n            }`)
                  }
      ...
      const essentialMetadataStr = meta.essentialConfigKeysMetadata
        .map((metaItem) => {
      ...
      if (metadata.globalConfigOptions && metadata.globalConfigOptions.length > 0) {
        const globalOptionsStr = metadata.globalConfigOptions
          .map((opt) => {
            let optStr = `        {\n          key: ${JSON.stringify(opt.key)},\n          label: ${JSON.stringify(opt.label)},\n          type: ${JSON.stringify(opt.type)}`
...
async function main() {
  ...
  cleanupUnusedTranslationFiles(Array.from(PLUGINS))
...
/**
 * Clean up unused translation files
 * Removes translation files for plugins that no longer exist
 */
function cleanupUnusedTranslationFiles(activePlugins: string[]): void {
  if (!fs.existsSync(PLUGINS_I18N_DIR)) {
    return
  }

  const existingPluginDirs = fs
    .readdirSync(PLUGINS_I18N_DIR, { withFileTypes: true })
    .filter((dirent) => dirent.isDirectory())
    .map((dirent) => dirent.name)

  const unusedPlugins = existingPluginDirs.filter((plugin) => !activePlugins.includes(plugin))

  if (unusedPlugins.length > 0) {
    console.log(`\n🧹 Cleaning up unused translation files...`)
    unusedPlugins.forEach((plugin) => {
      const pluginDir = path.join(PLUGINS_I18N_DIR, plugin)
      try {
        fs.rmSync(pluginDir, { recursive: true, force: true })
        console.log(`  🗑️  Removed translations for unused plugin: ${plugin}`)
      } catch (error) {
        console.warn(`  ⚠️  Could not remove translations for ${plugin}:`, error)
      }
    })
  }
}
