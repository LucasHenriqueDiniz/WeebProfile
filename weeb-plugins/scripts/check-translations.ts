...
function findUntranslatedKeys(
  ...
  const technicalTerms = [
    'Stargazers', 'Gists', 'Steam ID64', 'Steam ID', 'ID64',
    'ghp_', 'API', 'URL', 'PAT', 'token', 'key', 'GitHub',
    '16Personalities', 'LastFM', 'MyAnimeList', 'Stack Overflow',
    'Codeforces', 'Codewars', 'Duolingo', 'Lyfta', 'Steam'
  ]
  ...
  if (typeof enObj !== 'object' || typeof translatedObj !== 'object' || enObj === null || translatedObj === null) {
    ...
    if (typeof enObj === 'string' && typeof translatedObj === 'string') {
      ...
      if (!shouldSkip &&
          ...
          !enNormalized.match(/^[a-z_]+$/)) { // Ignora placeholders como "your-api-key"
...
async function main() {
  ...
  const partiallyTranslated = results.filter(r => 
    r.enExists && 
    r.locales.some(l => l.exists && !l.hasTranslation)
  ).length
  ...
  for (const result of results) {
    ...
    const localeStatus = result.locales.map(l => {
