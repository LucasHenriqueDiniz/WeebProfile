...
// Copy template
let templatePath = join(__dirname, "../src/plugins/_template")
if (!existsSync(templatePath)) {
  // Try alternative template path
  const altTemplatePath = join(__dirname, "../scripts/templates/plugin-template")
  if (!existsSync(altTemplatePath)) {
    console.error(`❌ Error: Template not found at ${templatePath} or ${altTemplatePath}`)
    process.exit(1)
  }
  // Use alt template path
  templatePath = altTemplatePath as string
}
...
if (existsSync(managerPath)) {
  ...
  if (!managerContent.includes(importLine)) {
    ...
    if (lastImportMatch) {
      ...
      managerContent = managerContent.slice(0, insertIndex) + "\n" + importLine + managerContent.slice(insertIndex)
    }
  }
  ...
  if (!managerContent.includes(registerLine)) {
    ...
    if (registerMatch) {
      ...
      managerContent = managerContent.slice(0, insertIndex) + "\n" + registerLine + managerContent.slice(insertIndex)
    }
  }
...
// Generate metadata and i18n keys automatically
console.log(`\n📝 Generating metadata and i18n keys...`)
try {
  ...
  console.log(`  ✓ Metadata and i18n keys generated automatically`)
} catch (error: any) {
...
console.log(`   5. Run pnpm generate-metadata to update centralized metadata and i18n keys`)
console.log(`   6. Test with pnpm dev`)
console.log(`\n🌍 Internationalization:`)
console.log(`   - i18n keys were automatically generated in weeb-dashboard/messages/plugins/${pluginName}/`)
console.log(`   - Edit en.json with English translations (required)`)
console.log(`   - Add pt.json and es.json for additional languages`)
console.log(`   - Missing translations will be flagged by validate-plugins`)
